    ...
    )
{
    ...
    //拡張子を除いたファイル名だけを取得(今回は、name + ".err"を利用したい)
    ...
    //.errファイルの例：左から順に「エラーの行」「左からの文字数」「エラーの種類」を表す番号となっている
    //test.err : 36 1 201
    //           88 29 144
    ...
    reader.on("line", (line:string) => {
        ...
        displayProblems(Number(errorLine),Number(errorColumn),Number(errorNumber),uri,diagnostics,diagnosticCollection);
        channel.appendLine(
            "*" + errorNumber + " : "  + queryMizarMsg(Number(errorNumber)) + " : " + "file:///" + fileName + "#" + errorLine
        );
    });
}
